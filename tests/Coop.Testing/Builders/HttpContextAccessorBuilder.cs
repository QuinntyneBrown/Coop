...
public class ClaimsPrincipalBuilder
{
    ...
    public ClaimsPrincipalBuilder WithUser(User user)
    {
        var claims = new List<Claim>
         {
             new (Coop.Domain.Constants.ClaimTypes.UserId, $"{user?.UserId}"),
             new (Coop.Domain.Constants.ClaimTypes.Username, $"{user?.Username}")
         };
        if (user != null)
        {
            foreach (var role in user.Roles)
            {
                ...
                {
                    claims.Add(new Claim(Coop.Domain.Constants.ClaimTypes.Privilege, $"{privilege.AccessRight}{privilege.Aggregate}"));
                }
            }
        }
