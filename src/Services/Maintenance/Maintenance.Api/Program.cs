...
using Maintenance.Infrastructure.Data.Seeding;
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "Maintenance Service", Version = "v1" });
    ...
    foreach (var arg in args)
    {
        switch (arg)
        {
            case "dropdb":
                ...
                Console.WriteLine("Maintenance database dropped.");
                break;
            case "migratedb":
                await context.Database.MigrateAsync();
                Console.WriteLine("Maintenance database migrated.");
                break;
            case "seeddb":
                MaintenanceSeedData.Seed(context);
                Console.WriteLine("Maintenance database seeded.");
                break;
            case "stop":
                return;
        }
